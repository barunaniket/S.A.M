...
def transition_meeting_status(meeting_id: str, new_status: str):
    ...
    if current_status is None:
        return {"success": False, "message": "Meeting does not exist"}

    allowed = VALID_TRANSITIONS.get(str(current_status), [])
    ...
    return {"success": True, "message": f"Transitioned to {new_status}"}
